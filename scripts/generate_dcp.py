...
def main():
    ...
    # -cinema2K/4K handles profiles, bitrate, and constraints automatically
    j2k_params = ["-OutFor", "j2c", "-threads", "16"]
    
    if "4K" in args.title or "4k" in args.title:
        # 4K DCI (only 24fps supported in standard DCI 4K, default in opj_compress)
        # Note: opj_compress -cinema4K does not accept an argument, defaults to 24fps
        j2k_params += ["-cinema4K"]
    else:
        # 2K DCI
        if args.fps == 48:
    ...
    if is_sequence:
        ...
        # With opj_compress cinema profiles, consistency is enforced by the profile
        consistent_params = base_params
        
        for i, img_path in enumerate(input_files):
            dst_j2k = os.path.join(j2k_dir, f"frame_{i:06d}.j2c")
            cmd_convert = ["opj_compress", "-i", img_path, "-o", dst_j2k] + consistent_params
        ...
        cmd_convert = ["opj_compress", "-i", input_files[0], "-o", base_j2k] + j2k_params
